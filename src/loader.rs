...
fn name_atom(filename) -> {
    /* If there is an extension remove it */
    Value::Atom(Path::from(filename).file_stem().unwrap())
    Value.AtomValue (try
...
/* See also path.ml */
lazy_static! {
    static ref DEFAULT_PACKAGE_PATH: PathBuf = {
        let env_path = PathBuf::from(env!("BUILD_PACKAGE_DIR"));
        path::executable_relative_path(env_path)
    };
}

fn package_root_path() -> PathBuf {
    match options::RUN.read().unwrap().package_path {
        Some(s) => s.clone(),
        None => DEFAULT_PACKAGE_PATH.clone(),
    }
}
...
fn self_filter(self: Value, source: LoaderSource) -> Self {
    match source {
...
fn known_filter(source: LoaderSource) -> Option<Value> {
    match source {
...
lazy_static! {
    static ref PACKAGES_LOADED: HashMap<> = HashMap::new();
}
