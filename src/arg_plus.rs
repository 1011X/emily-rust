...
G: Fn(Vec<String>) {
	...
	let proceed = || while let Some(key) = rest.next() { /* Argument found */
		match lookup.get(&key) {
			...
			Some(Spec::String(f)) => return rest.next()
				.ok_or(Error::Bad(format!("option '{}' needs an argument.", key)))
				.map(f),
				...
				if key.len() > 0 && key.starts_with('-') {
					match key.find('=') {
						...
						Some(split_at) => {
							...
							let sub_value = key[split_at + 1..].to_owned();
	...
	let name = rest.next().unwrap_or("(INTERNAL ERROR)".to_owned());
